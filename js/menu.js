import { MenuAPI, escapeHTML } from './api.js'; import { UI } from './ui.js';
const SukidesuMenu = {
  ...
  openModal() { document.getElementById("order-modal").classList.remove("hidden"); }, closeModal() { document.getElementById("order-modal").classList.add("hidden"); },
  updateUI() {
  ...
  async init() {
    ...
    try {
      ...
      // BARRERA DE PAGO: Verifica si el servicio está suspendido
      if (this.config.estado_servicio && String(this.config.estado_servicio).toLowerCase() === "suspendido") {
        ...
        return; // Detiene la ejecución para no cargar platos
      }
